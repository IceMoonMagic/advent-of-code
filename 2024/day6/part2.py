...
def check_for_loop(map_: list[str], pos: Position, facing: Facing) -> bool:
    visited: set[tuple[Position, Facing]] = set()
    while pos is not None:
        if (pos, facing) in visited:
            return True
        visited.add((pos, facing))
        pos, facing = preform_next_action(map_, pos, facing)
    return False
...
def main():
    ...
    for y in range(len(data)):
        for x in range(len(data[y])):
            ...
            looping_obstructions += check_for_loop(data, pos, facing)
            data[y] = data[y][:x] + "." + data[y][x + 1 :]
    print(looping_obstructions)
